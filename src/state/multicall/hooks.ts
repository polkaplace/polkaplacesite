...
function useCallsData(calls: (Call | undefined)[], options?: ListenerOptions): CallResult[] {
  ...
  useEffect(() => {
    ...
    dispatch(
      addMulticallListeners({ chainId, calls, options })
    )

    return () => {
      dispatch(
        removeMulticallListeners({chainId, calls, options })
      )
    }
...
export function useMultipleContractSingleData(addresses: (string | undefined)[],contractInterface: Interface,methodName: string,callInputs?: OptionalMethodInputs,options?: ListenerOptions): CallState[] {
